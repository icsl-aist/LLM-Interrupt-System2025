...
| `config.py` | 【設定】IPアドレス、APIキー、ファイルパスなどのシステム全体設定 |
...
| `task_en.json` / `task.json` | 行動生成用プロンプトの旧バージョンまたは別バージョン |
| `talk_en.json` / `talk.json` | 会話生成用プロンプトの旧バージョンまたは別バージョン |
| `user_task.txt` | ユーザー入力タスクの一時保存用テキスト |
...
| `speak_.py` | 発話機能の単体テスト用モジュール |
| `z_house.py` / `z_home_*.py` | 位置情報やホームポジション定義の補助ファイル |
...
| `*.txt` (a, ak, k, kari...) | テスト用または一時保存用のスクリプト |
